...
export const ProductForm: React.FC<ProductFormProps> = ({
    ...
    // Some toggler variales between new product creation or existing product updation mode
    ...
    const form = useForm<ProductFormValues>({
        ...
        defaultValues: initialData ? {
            ...initialData,
            price: parseFloat(String(initialData?.price)), // We have to modify the price as in db it is in decimal but here it is in float or number
        } : {
            name: '',
            images: [],
            price: 0,
    ...
    const onSubmit = async (data: ProductFormValues) => {
        try {
            setLoading(true);
            if (initialData) {
                await axios.patch(`/api/${params.storeId}/products/${params.productdId}`, data);
            } else {
